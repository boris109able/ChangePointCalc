 SGLmain <- function(data, gamma = 0.9, maxit = 1000, thresh = 0.001, min.frac = 0.05, nlam = 20, alpha = 0.8, standardize = TRUE, verbose = FALSE, step = 1, reset = 10, lambdas = NULL)
{
   n <- nrow(data$x)
   p <- ncol(data$x)
   np <- n*p
  ...
  Sol <- SGLcalc(data, thresh=thresh, inner.iter = maxit, outer.iter = maxit, outer.thresh = thresh, min.frac = min.frac, nlam = nlam, lambdas = lambdas, gamma = gamma, verbose = verbose, step = step, reset = reset, alpha = alpha)
  if(standardize == TRUE){
